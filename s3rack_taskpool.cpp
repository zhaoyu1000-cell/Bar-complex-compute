...
// Build: g++ -O3 -std=c++20 -march=native -pipe -pthread s3rack_taskpool.cpp -o
// s3rack_taskpool Run:   ./s3rack_taskpool --n 6 --root-order 1 --sign 1
// --prime 1009 --threads 8
//        ./s3rack_taskpool --n 6 --root-order 1 --sign -1 --prime 1009
//        --threads 8
//        ./s3rack_taskpool --n 6 --root-order 12 --root-exp 1 --sign 1
//        --threads 8
...
static int q_scalar_in_gfp(int p, int m, int exp, int sign) {
  ...
  if (m == 1)
    return (sign == +1) ? 1 : (p - 1);
  if (p % m != 1)
    throw std::runtime_error("Need p ≡ 1 (mod m)");
  int g = primitive_root_mod_prime(p);
  int zeta = mod_pow(g, (p - 1) / m, p);
  int q = mod_pow(zeta, ((exp % m) + m) % m, p);
  if (sign == -1)
    q = (p - q) % p;
  return q;
}
...
static inline int encode3(const uint8_t *d, int L,
...
static inline void apply_swaps_rack(uint8_t *d, int L,
...
precompute_qshuffle(int n, int p, int q_scalar, const uint8_t op[3][3],
                    const std::vector<int> &pow3) {
  ...
  for (int L = 1; L <= n; L++) {
    ...
    for (int wid = 0; wid < pow3[L]; wid++)
      decode3(wid, L, digits[L][wid].data());
  }
...
// -------------------- sparse elimination --------------------
using SV = std::vector<std::pair<int, int>>;
...
static SparseRectMatrix compile_block_differential_matrix(
    int n, int k, int g, int p, const std::vector<std::vector<QShufflePQ>> &qsh,
    const std::vector<int> &pow3, const MonoBlocks &mb) {
  ...
  for (int i = 0; i < (int)comps_t.size(); i++)
    pos_t[comps_t[i]] = i;
      ...
      for (const auto &ms : sp) {
        ...
        for (const auto &ow : lst) {
          ...
          if (!val)
            continue;
          int new_wid =
              replace_segment(wid, ms.start, ms.out_len, out_id, pow3);
...
static int rank_rectangular_wiedemann(const SparseRectMatrix &d, int p,
  ...
  for (int c = 0; c < d.cols; ++c) {
    for (const auto &[r, _] : d.col_vectors[c])
      row_nnz[r]++;
  }
  for (int r = 0; r < d.rows; ++r)
    sq[r].reserve(row_nnz[r]);
...
int main(int argc, char **argv) {
  ...
  const int q_scalar = q_scalar_in_gfp(p, m, exp, sign);
  ...
  int trans_gid[3] = {perm_id[keyperm(T12)], perm_id[keyperm(T13)],
                      perm_id[keyperm(T23)]};
  ...
  for (int x = 0; x < 3; x++)
    for (int y = 0; y < 3; y++) {
      Perm z =
          compose_perm(compose_perm(transP[x], transP[y]), inv_perm(transP[x]));
      int idx = 0;
      for (int t = 0; t < 3; t++)
        if (transP[t] == z)
          idx = t;
  ...
  for (int k = 2; k <= n; k++) {
    ...
    for (const Task &t : tasks) {
      SparseRectMatrix d_block =
          compile_block_differential_matrix(n, k, t.g, p, qsh, pow3, mb);
      std::uint32_t seed = 2166136261u ^ (std::uint32_t)n * 16777619u ^
                           (std::uint32_t)k * 1315423911u ^
                           (std::uint32_t)t.g * 2654435761u;
    ...
    std::cout << "[progress] finished d_" << k << " rank=" << rank_d[k]
