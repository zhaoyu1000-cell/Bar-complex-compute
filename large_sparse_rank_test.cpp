...
using SparseRow = std::map<int, int>;
using SparseMatrix = std::vector<SparseRow>;
using SparsePairMatrix = sparse_wiedemann::SparsePairMatrix;

static SparseMatrix generate_sparse_full_rank_matrix(int n, int max_per_row,
                                                     int p, std::mt19937 &rng) {
...
int main(int argc, char **argv) {
  ...
  std::cout << "Parallel Wiedemann-only test: k=" << K << ", size=" << N << "x"
            << N << ", repeats=" << WIEDEMANN_REPEATS << "\n";

  for (int t = 1; t <= trials; ++t) {
    auto gen0 = std::chrono::steady_clock::now();
    SparseMatrix mat =
        generate_sparse_full_rank_matrix(N, MAX_NONZERO_PER_ROW, P, rng);
    SparsePairMatrix pair_sparse = map_sparse_to_pair_sparse(mat);
    auto gen1 = std::chrono::steady_clock::now();

    int gen_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(gen1 - gen0)
            .count();
    ...
    for (int threads : thread_counts) {
      ...
      int rank = sparse_wiedemann_parallel::rank_probabilistic_parallel(
          pair_sparse, P, WIEDEMANN_REPEATS, threads,
          static_cast<std::uint32_t>(rng()));
      auto t1 = std::chrono::steady_clock::now();
      int ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0)
                   .count();
