...
static int rank_mod_p(vector<vector<int>> matrix, int p) {
  ...
  for (int col = 0; col < m && row < n; ++col) {
    ...
    const int pivot_value = normalize_mod(matrix[row][col], p);
    const int inv_pivot = mod_inverse(pivot_value, p);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      const int factor = normalize_mod(matrix[i][col], p);
      if (factor == 0) {
        continue;
      }
