...
static std::vector<Int> apply_sparse_matrix(const SparsePairMatrix &a,
                                            const std::vector<Int> &x, Int p) {
...
static std::vector<Int> apply_sparse_matrix_transpose(const SparsePairMatrix &a,
...
static int berlekamp_massey_linear_complexity(const std::vector<Int> &sequence,
                                              Int p) {
  ...
  auto mod_inv = [&](Int x) {
    x = normalize_mod(x, p);
    if (x == 0)
      throw std::invalid_argument("Berlekamp-Massey division by zero");
...
int rank_probabilistic(const SparsePairMatrix &a, Int p, URBG &rng,
                       int repeats = 3) {
  const int n = static_cast<int>(a.size());
  if (n == 0)
    return 0;
