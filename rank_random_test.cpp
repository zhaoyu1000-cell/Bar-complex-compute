...
static Matrix generate_full_row_rank_basis(int r, int m, int p,
                                           std::mt19937 &rng) {
  std::uniform_int_distribution<int> entry_dist(0, p - 1);

  Matrix basis(r, std::vector<int>(m));
  if (r == 0) {
    return basis;
  }
...
static Matrix generate_rank_r_matrix(int n, int m, int r, int p,
                                     std::mt19937 &rng) {
  ...
  std::uniform_int_distribution<int> coeff_dist(0, p - 1);
...
static SparsePairMatrix dense_to_sparse_square(const Matrix &a, int p) {
  int n = static_cast<int>(a.size());
  SparsePairMatrix sparse(n);
  for (int i = 0; i < n; ++i) {
    int m = static_cast<int>(a[i].size());
    for (int j = 0; j < m; ++j) {
      int v = normalize_mod(a[i][j], p);
      if (v != 0)
        sparse[i].push_back({j, v});
    }
  }
  return sparse;
}

static Matrix generate_random_sparse_square(int n, int max_per_row, int p,
                                            std::mt19937 &rng) {
  ...
  std::uniform_int_distribution<int> val_dist(1, p - 1);
  for (int i = 0; i < n; ++i) {
    int cnt = count_dist(rng);
    std::map<int, int> row_entries;
...
int main() {
  ...
  constexpr int P = 1009; // prime modulus
  ...
  for (int t = 1; t <= TRIALS; ++t) {
    ...
    if (computed_rank != r) {
      std::cerr << "Trial " << t << " failed: expected rank " << r << ", got "
                << computed_rank << '\n';
      return EXIT_FAILURE;
    }
  ...
  for (int t = 1; t <= TRIALS; ++t) {
    ...
    int wiedemann_rank =
        sparse_wiedemann::rank_probabilistic(sparse, P, rng, 5);

    if (wiedemann_rank != exact_rank) {
      std::cerr << "Wiedemann sparse trial " << t << " failed: expected rank "
                << exact_rank << ", got " << wiedemann_rank << '\n';
      return EXIT_FAILURE;
    }

    std::cout << "Wiedemann sparse trial " << t
              << " passed (rank = " << exact_rank << ")\n";
  }
