...
int main(int argc,char** argv){
 ...
 std::vector<std::atomic<double>> time_sum(n+2); for(int i=0;i<n+2;i++) time_sum[i].store(0.0);
 ...
 for(int k=2;k<=n;k++){ rank_d[k]=(__int128)rank_sum[k].load(std::memory_order_relaxed); time_d[k]=time_sum[k].load(std::memory_order_relaxed); std::cout<<"rank(d_"<<k<<")="<<(long long)rank_d[k]<<" time="<<time_d[k]<<"s\n"; }
