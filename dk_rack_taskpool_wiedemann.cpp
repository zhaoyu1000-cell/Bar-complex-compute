...
struct Dihedral {
  int N;
  int G;
  explicit Dihedral(int n) : N(n), G(2 * n) {};
  inline int gid(int a, int b) const {
    a %= N;
    if (a < 0)
      a += N;
    b &= 1;
  ...
  inline int a_of(int g) const { return g % N; }
  inline int b_of(int g) const { return g / N; }
  inline int id() const { return gid(0, 0); }
  inline int mul(int g1, int g2) const {
    int a1 = a_of(g1), b1 = b_of(g1), a2 = a_of(g2), b2 = b_of(g2);
    int a = (b1 ? (a1 - a2) : (a1 + a2)) % N;
    if (a < 0)
      a += N;
    return gid(a, b1 ^ b2);
  }
  inline int inv(int g) const {
    int a = a_of(g), b = b_of(g);
    if (b == 0)
      return gid(-a, 0);
    return g;
  }
  inline int conj(int x, int y) const { return mul(mul(x, y), inv(x)); }
...
static std::vector<std::vector<int>> shuffle_swaplists(int p_len, int q_len) {
  int L = p_len + q_len;
  std::vector<std::vector<int>> res;
  std::vector<int> bit(L, 0);
  std::fill(bit.begin(), bit.begin() + p_len, 1);
  std::sort(bit.begin(), bit.end(), std::greater<int>());
  do {
    std::vector<int> perm;
    int iu = 0, iv = 0;
    for (int i = 0; i < L; i++)
      perm.push_back(bit[i] ? iu++ : p_len + iv++);
    res.push_back(reduced_swaps_for_perm(perm));
...
static void comps_rec(int n, int k, std::vector<int> &cur,
...
struct KData {
  int k = 0;
  std::vector<std::vector<int>> comps_k, comps_t;
...
static KData precompute_kdata(int n, int k, int64_t P) {
  ...
  kd.comps_t = compositions(n, k - 1);
  std::unordered_map<std::vector<int>, int, VecHash> pos_t;
  for (int i = 0; i < (int)kd.comps_t.size(); i++)
    pos_t[kd.comps_t[i]] = i;
  kd.specs.assign(kd.comps_k.size(), {});
  std::unordered_map<int, bool> seen;
  for (int ci = 0; ci < (int)kd.comps_k.size(); ci++) {
    const auto &comp = kd.comps_k[ci];
    auto cuts = cuts_for_comp(comp);
    std::vector<Spec> sp;
    for (int i = 1; i <= k - 1; i++) {
      int start = cuts[i - 1], p_len = comp[i - 1], q_len = comp[i],
          out_len = p_len + q_len;
      ...
      if (!seen[key]) {
        seen[key] = true;
        kd.needed_pairs.push_back({p_len, q_len});
      }
    }
    kd.specs[ci] = std::move(sp);
  }
  return kd;
}
...
static MonoBlocks monodromy_blocks_Dn(int n, int base,
  ...
  for (int64_t wid = 0; wid < Nwords; wid++) {
    ...
    int idx = (int)mb.words_by_g[g].size();
    mb.idx_in_block[(size_t)wid] = idx;
...
static ConjClasses conjugacy_classes(const Dihedral &D) {
  int G = D.G;
  ConjClasses cc;
  cc.class_of.assign(G, -1);
  for (int g = 0; g < G; g++) {
    if (cc.class_of[g] != -1)
      continue;
    std::vector<char> in(G, 0);
    for (int h = 0; h < G; h++)
      in[D.conj(h, g)] = 1;
    std::vector<int> cls;
    int rep = -1;
    ...
    int cid = (int)cc.classes.size();
    cc.classes.push_back(std::move(cls));
    cc.rep.push_back(rep);
    for (int x : cc.classes.back())
      cc.class_of[x] = cid;
  }
  return cc;
}

struct SparseWiedemannInput {
  sparse_wiedemann::SparsePairMatrix sq;
};
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    auto need = [&](const char *opt) {
      if (i + 1 >= argc)
        throw std::runtime_error(std::string("Missing ") + opt);
      return std::string(argv[++i]);
    };
    if (s == "--dihedral")
      dihedralN = std::stoi(need("--dihedral"));
    else if (s == "--n")
      n = std::stoi(need("--n"));
    else if (s == "--threads")
      threads = std::stoi(need("--threads"));
    else if (s == "--only-k")
      only_k = std::stoi(need("--only-k"));
    else if (s == "--root-order")
      m = std::stoll(need("--root-order"));
    else if (s == "--root-exp")
      exp = std::stoll(need("--root-exp"));
    else if (s == "--sign")
      sign = std::stoi(need("--sign"));
    else if (s == "--prime")
      P = std::stoll(need("--prime"));
    else if (s == "--repeats")
      repeats = std::stoi(need("--repeats"));
    else if (s == "--nested-blocks")
      nested_blocks = std::stoi(need("--nested-blocks"));
    else
...
#endif
  std::cout << "D_" << dihedralN << " n=" << n << " P=" << P
            << " q=" << q_scalar << " repeats=" << repeats << "\n";
  std::vector<std::vector<uint8_t>> op(base, std::vector<uint8_t>(base, 0));
  for (int i = 0; i < base; i++) {
    int xi = D.reflection_gid(i);
    for (int j = 0; j < base; j++) {
      int z = D.conj(xi, D.reflection_gid(j));
      op[i][j] = (uint8_t)D.a_of(z);
    }
  }
  MonoBlocks mb = monodromy_blocks_Dn(n, base, powB, D);
  ConjClasses cc = conjugacy_classes(D);
  std::vector<KData> kd(n + 1);
  for (int k = 2; k <= n; k++)
    kd[k] = precompute_kdata(n, k, P);
  ...
  tasks.reserve((size_t)(n - 1) * cc.classes.size());
  for (int k = 2; k <= n; k++)
    for (int cid = 0; cid < (int)cc.classes.size(); cid++) {
      int rep = cc.rep[cid];
      const int block_words = (int)mb.words_by_g[rep].size();
      if (block_words == 0)
        continue;
  ...
  std::sort(tasks.begin(), tasks.end(), [](const Task &a, const Task &b) {
    ...
    if (a.k != b.k)
      return a.k > b.k;
    return a.rep_g < b.rep_g;
  });
  std::atomic<int> next(0);
  int outer_worker_count = nested_blocks ? std::max(1, threads / 3) : threads;
  outer_worker_count =
      std::max(1, std::min(outer_worker_count, (int)tasks.size()));
  int inner_threads_per_worker =
      nested_blocks ? 2 : std::max(1, threads / outer_worker_count);
  ...
  pool.reserve(outer_worker_count);
  for (int t = 0; t < outer_worker_count; t++)
    pool.emplace_back(worker);
  for (auto &th : pool)
    th.join();
  std::vector<__int128> rank_d(n + 2, 0);
  std::vector<double> time_d(n + 2, 0.0);
  for (int k = 2; k <= n; k++) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            << "qshuffle_pre=" << std::chrono::duration<double>(t1 - t0).count()
            << "s total_rank_time="
            << std::chrono::duration<double>(t2 - t1).count() << "s\n";
  std::cout << "\nH^j dims (j=0..n-1):\n";
  for (int j = 0; j < n; j++) {
    int k = n - j;
    __int128 dimBk = binom_i128(n - 1, k - 1) * (__int128)powB[n];
    __int128 Hj = dimBk - rank_d[k] - rank_d[k + 1];
    std::cout << "H^" << j << " = ";
    print_i128(Hj);
    std::cout << "\n";
  }
