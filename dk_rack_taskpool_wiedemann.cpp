...
int main(int argc,char** argv){
 ...
 for(int k=2;k<=n;k++){ auto ks=std::chrono::high_resolution_clock::now(); __int128 sum=0; for(int cid=0; cid<(int)cc.classes.size(); cid++){ int rep=cc.rep[cid]; if(mb.words_by_g[rep].empty()) continue; auto mat=compile_block_differential_matrix(n,P,qsh,powB,mb,kd[k],rep); std::uint64_t seed=1469598103934665603ULL ^ (std::uint64_t)k*1315423911ULL ^ (std::uint64_t)rep*2654435761ULL; int rk=rank_rectangular_wiedemann(mat,P,repeats,threads,seed); sum += (__int128)rk * (__int128)cc.classes[cid].size(); }
