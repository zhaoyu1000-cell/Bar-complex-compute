...
int main(int argc,char** argv){
 ...
 for(int k=2;k<=n;k++){ auto ks=std::chrono::high_resolution_clock::now(); std::vector<std::pair<int,int>> tasks; for(int cid=0; cid<(int)cc.classes.size(); cid++){ int rep=cc.rep[cid]; if(!mb.words_by_g[rep].empty()) tasks.push_back({rep,(int)cc.classes[cid].size()}); } std::atomic<int> next(0); std::atomic<long long> sum_ll(0); int worker_count=std::max(1,std::min(threads,(int)tasks.size())); auto worker=[&](){ while(true){ int idx=next.fetch_add(1,std::memory_order_relaxed); if(idx>=(int)tasks.size()) return; int rep=tasks[idx].first; int class_sz=tasks[idx].second; auto mat=compile_block_differential_matrix(n,P,qsh,powB,mb,kd[k],rep); std::uint64_t seed=1469598103934665603ULL ^ (std::uint64_t)k*1315423911ULL ^ (std::uint64_t)rep*2654435761ULL; int rk=rank_rectangular_wiedemann(mat,P,repeats,threads,seed); sum_ll.fetch_add((long long)rk*(long long)class_sz,std::memory_order_relaxed); }}; std::vector<std::thread> pool; pool.reserve(worker_count); for(int t=0;t<worker_count;t++) pool.emplace_back(worker); for(auto& th:pool) th.join(); __int128 sum=(__int128)sum_ll.load(std::memory_order_relaxed);
 auto ke=std::chrono::high_resolution_clock::now(); rank_d[k]=sum; time_d[k]=std::chrono::duration<double>(ke-ks).count(); std::cout<<"rank(d_"<<k<<")="<<(long long)sum<<" time="<<time_d[k]<<"s\n"; }
